...
object Box {
  ...
  def toss(box: Box): Box = Box(box.a + (box.b + box.c) / 2, (box.b + box.c) / 2, (box.c - box.b) / 2)
  ...
  def moveHorizontally(offset: Double)(box: Box): Box = Box(box.a + box.b * offset, box.b, box.c)

  def moveVertically(offset: Double)(box: Box): Box = Box(box.a + box.c * offset, box.b, box.c)
